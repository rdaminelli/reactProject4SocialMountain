...
const Profile = () => {
    ...
    const getUserPosts = useCallback(() => {
        axios.get(`http://localhost:3001/app/userposts/${userId}`)
    ...
    const updatePost = (id, status) => {
        axios.put(`http://localhost:3001/app/posts/${id}`, {status: !status}, {
            headers: {
                authorization: token
            }
        })
    ...
    const deletePost = id => {
        axios.delete(`http://localhost:3001/app/posts/${id}`, {
            headers: {
                authorization: token
            }
        })
